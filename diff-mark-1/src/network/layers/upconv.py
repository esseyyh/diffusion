...
class UpBlock(nn.Module):
    def __init__(self,fan_in,fan_out,num_filters=3,time_embedding_dims=128):
        super().__init__()
        self.conv1 = nn.Conv2d(2*fan_in, fan_out, num_filters, padding=1)
        ...
        self.time_embedding = SPE(time_embedding_dims)
        self.mlp=nn.Linear(time_embedding_dims,fan_out)
    def forward(self,x,time):
        x=self.bnorm1(self.relu(self.conv1(x)))
        x_time = self.relu(self.mlp(self.time_embedding(time)))
        x= x + x_time[(..., ) + (None, ) * 2]
